...
public class ReactController {
	@Autowired
	private ChefDAO dao;
	@RequestMapping(value="react_mangochef/chef_list.do",produces="text/plain;charset=UTF-8")
	...
	@RequestMapping("react_mangochef/totalpage.do")
	...
	@RequestMapping(value="react_mangochef/chef_detail.do",produces="text/plain;charset=UTF-8")
	public String chef_detail(String chef,String page)
	{
		...
		if(page==null)
			page="1";
		int curpage=Integer.parseInt(page);
		List<ChefRecipeVO> list=dao.chefProductData(chef, curpage);
		try
		{
			JSONArray arr=new JSONArray();
			//[{"poster":"",title:"",chef:""},{},{},{}]
			for(ChefRecipeVO vo:list)
			{
				JSONObject obj=new JSONObject();
				obj.put("poster", vo.getPoster());
	...
	@RequestMapping(value="react_mangochef/chef_find.do",produces="text/plain;charset=UTF-8")
	public String chef_find(String chef,String fd)
	{
		List<ChefRecipeVO> list=dao.chefProductFindData(chef, fd);
		String json="";
		try
		{
			JSONArray arr=new JSONArray();
	...
	public String recipe_list(String page)
	{
		if(page==null)
			page="1";
		int curpage=Integer.parseInt(page);
		List<ChefRecipeVO> list=dao.recipeListData(curpage);
		String json="";
		try
		{
			/*
